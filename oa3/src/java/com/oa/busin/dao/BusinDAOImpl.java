...
implements BusinDAO {
    public String getHql(VBusin query, Object[] obj, int flag) {
        ...
        if (query != null) {
            ...
            if (StringUtils.isNotBlank(query.getPickupStatus())) {
            	whereStr.append(" and busin.pickupStatus = '").append(query.getPickupStatus()).append("'");
            	whereStr.append(" and busin.typeId = 13");
            }
        ...
        if (obj != null && obj.length > 0) {
            ...
            if (obj.length >= 9) {
            	if(!"1".equals(String.valueOf(obj[8]))) {
            		whereStr.append(" and busin.salerId in (select saler.CId from TSaler saler where saler.user.CId = ").append(UserInfo.getUserId()).append(")");
            	}
            }
        }
        if (!lock) {
    ...
    public TBusin getBusinByBillNo(String billNo) {
    	String hql = "from TBusin t where t.CBillNo = ?";
    	List list = getHibernateTemplate().find(hql, billNo);
    	if (list != null && list.size() > 0) {
            return (TBusin)list.get(0);
        }
        return null;
    ...
    public String getBusinSumReportHql(VBusinSumReport query, Object[] obj, int flag) {
        ...
        if (obj != null && obj.length > 0) {
            if (obj[0] != null && !"".equals(obj[0].toString().trim())) {
                where.append(" and vv.nodate4 >= '").append(obj[0].toString()).append(" 00:00:00'");
            }
            if (obj[1] != null && !"".equals(obj[1].toString().trim())) {
                where.append(" and vv.nodate4 <= '").append(obj[1].toString()).append(" 23:59:59'");
            }
        }
        if (where != null && !"".equals(where.toString().trim())) {
            hql.append(" where ").append(where.toString().substring(4));
