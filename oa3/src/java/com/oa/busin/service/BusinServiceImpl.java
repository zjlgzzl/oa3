...
implements BusinService {
    ...
    public void saveEntity(TBusin busin, List<TContainer> cons, List<TService> services, List<TCost> costs, List<TCost> costsGroup, List<TCash> cash, int[] delCost, String[] logs,Map<Integer, String> map) throws Exception {
        List serTmpList;
        List conTmpList;
        TBusinType tmp;
        int businTypeId;
        ...
        if (costsGroup != null && costsGroup.size() > 0) {
            ...
            for (int i = 0; i < costsGroup.size(); ++i) {
                ...
                costsGroup.get(i).setAgent(map.get(costsGroup.get(i).getTCostGroup().getCId()));
                if(StringUtils.isNotBlank(busin.getPayDateStr())) {
                	costsGroup.get(i).setPayDate(formatter3.parse(busin.getPayDateStr()));
                }else {
                	costsGroup.get(i).setPayDate(null);
                }
        ...
        if (costs != null && costs.size() > 0) {
            ...
            for (int i = 0; i < costs.size(); ++i) {
                ...
                if(StringUtils.isNotBlank(costs.get(i).getPayDateStr())) {
                	costs.get(i).setPayDate(formatter3.parse(costs.get(i).getPayDateStr()));
                }else {
                	costs.get(i).setPayDate(null);
                }
    ...
    public void saveCash(int businId, List<TContainer> cons, List<TCost> costs, List<TCost> costsGroup, List<TCash> cash, List<TCashRate> rate, List<TCash> cash2, List<TCashRate> rate2, short state, short flag, TBusin bb, String[] logs,Map<Integer, String> map) throws Exception {
        TJiezhuan jj;
        int i;
        List cashTmpList;
        StringBuffer logCostinfo = new StringBuffer();
        ...
        if (costs != null && costs.size() > 0) {
            ...
            for (int i4 = 0; i4 < costs.size(); ++i4) {
                ...
                if(StringUtils.isNotBlank(costs.get(i4).getPayDateStr())) {
                	costs.get(i4).setPayDate(formatter3.parse(costs.get(i4).getPayDateStr()));
                }else {
                	costs.get(i4).setPayDate(null);
                }
        ...
        if (costsGroup != null && costsGroup.size() > 0) {
            ...
            for (int i6 = 0; i6 < costsGroup.size(); ++i6) {
                ...
                costsGroup.get(i6).setAgent(map.get(costsGroup.get(i6).getTCostGroup().getCId()));
                if(StringUtils.isNotBlank(bb.getPayDateStr())) {
                	costsGroup.get(i6).setPayDate(formatter3.parse(bb.getPayDateStr()));
                }else {
                	costsGroup.get(i6).setPayDate(null);
                }
	...
	public void updateBusinPickupStatus(int businId) throws Exception {
		TBusin busin = businDAO.getEntityById(businId);
		busin.setCPickupStatus("1");
