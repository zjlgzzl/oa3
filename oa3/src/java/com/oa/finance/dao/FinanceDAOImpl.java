...
implements FinanceDAO {
    public String getHql(VFinance query, Object[] obj, int flag) {
        ...
        if (query != null) {
            ...
            if (query.getDescription() != null && !"".equals(query.getDescription().trim())) {
            	whereStr.append(" and vv.description like '%").append(query.getDescription().trim()).append("%'");
            }
    ...
    public int getNoticeCount() {
        StringBuffer sql = new StringBuffer();
        sql.append("select count(*) from t_finance f where exists").append(" (select 1 from t_finance_detail d where f.c_id = d.c_pid and d.c_item = '临时支出' and d.c_notice = 1)");
