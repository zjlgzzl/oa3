  ...
  <head>
	...
	<script type="text/javascript">
		...
		function forward(flag){
			...
			document.getElementById("queryForm").action = "${ctx}/busin/CostGroupList.action";
		...
		function view(cid){
			...
			document.getElementById("queryForm").action = "${ctx}/busin/ViewCostGroup.action";
			document.getElementById("queryForm").submit();
		}
		//添加
		function add(){
			document.getElementById("queryForm").action = "${ctx}/busin/AddCostGroup.action";
			document.getElementById("queryForm").submit();
		}
		//修改
		function edit(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/busin/EditCostGroup.action";
			document.getElementById("queryForm").submit();
		}
		//删除确认
		function confirmDel(cid){
			if (confirm("删除的数据将无法恢复，是否确认删除？")){
				document.getElementById("cid").value = cid;
				document.getElementById("queryForm").action = "${ctx}/busin/DeleteCostGroup.action";
				document.getElementById("queryForm").submit();
			}
			return false;
		}
			...
			<table style="width:440px; margin-left:20px;">
				<tr>
					...
					<td style="width:150px;text-align:left; ">
						<input name="cgroupQuery.CName" value="${requestScope.cgroupQuery.CName}" 
  					...
  					<td style="width:200px;text-align:left; ">
						...
						<input type="button" value="添加" class="btn_img" style="margin-left:10px;"
									onclick="add();"/>
						<input id="cid" name="cgroupId" type="hidden"/>
        ...
        <div class="right_3">
        	<table id="table1" class="table" style="width:720px;margin-left:20px;">
	           	<tr>
	               	...
	               	<th style="width:260px;">成本项目组名称</th>
