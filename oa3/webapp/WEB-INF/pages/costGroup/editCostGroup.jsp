						...
						<td>
							<select id="agent" name="cgroup.agent" class="field border" style="width:485px">
								...
								<c:forEach items="${agentList }" var="agent">
									<option value="${agent.name }" <c:if test="${cgroup.agent == agent.name }">selected</c:if>>${agent.name }</option>
								</c:forEach>
							</select>
						</td>
					</tr>
					...
					<tbody id="detail">
						...
						<s:iterator value="#request.cgroupItems" var="cgroupItems" status="num">
							<tr id="tr<s:property value="#num.index" />">
								...
								<td>
									<input id="order<s:property value="#num.index" />"
											name="cgroupItems[<s:property value="#num.index" />].COrder"
											value="${cgroupItems[num.index].COrder}"
											class="border field" style="width:90px;text-align:center;" />
								</td>
								<td>
