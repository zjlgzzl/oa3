...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript">
		...
		//添加
		function add(){
			document.getElementById("queryForm").action = "${ctx}/hscode/AddHscode2.action";
			document.getElementById("queryForm").submit();
		}
		//修改
		function edit(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/hscode/EditHscode2.action";
			document.getElementById("queryForm").submit();
		}
		//删除确认
		function confirmDel(cid){
			if (confirm("删除的数据将无法恢复，是否确认删除？")){
				document.getElementById("cid").value = cid;
				document.getElementById("queryForm").action = "${ctx}/hscode/DeleteHscode2.action";
				document.getElementById("queryForm").submit();
			}
			return false;
		}
		...
		function view(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/hscode/ViewHscode2.action";
			document.getElementById("queryForm").submit();
		}
	</script>
  </head>
  <body>
  	<div class="right">
  		<div class="right_1">
			<form id="queryForm" action="${ctx}/hscode/HsCodeList2.action" method="post">
				<table style="width:800px; margin-left:0;">
					<tr>
						<td style="width:90px;">Description</td>
						<td style="width:80px;text-align:left; ">
							<input name="query.CDescription" value="${requestScope.query.CDescription}" 
									type="text" class="field border" style="width:80px;" />
	  					</td>
	  					<td style="width:70px;">Details</td>
						<td style="width:80px;text-align:left; ">
							<input name="query.CDetails" value="${requestScope.query.CDetails}" 
									type="text" class="field border" style="width:80px;" />
	  					</td>
	  					<td style="width:70px;">柬文品名</td>
						<td style="width:80px;text-align:left; ">
							<input name="query.CName" value="${requestScope.query.CName}" 
									type="text" class="field border" style="width:80px;" />
	  					</td>
	  					<td style="width:70px;">HS Code</td>
						<td style="width:80px;text-align:left; ">
							<input name="query.CHscode" value="${requestScope.query.CHscode}" 
									type="text" class="field border" style="width:80px;" />
	  					</td>
	  					<td>
       	...
       	<div class="right_2">
       		<a href="${ctx}/hscode/AddHscode2.action" style="margin-left:900px;">添加</a>
        </div>
        <div class="right_3">
	        <table id="table1" class="table" style="width:950px;">
	            ...
	            <tr>
					...
					<td><s:property value="CDetails"/></td>
					<td><s:property value="CName"/></td>
					<td><s:property value="CHscode"/></td>
					<td><s:property value="TUser.CUsername"/></td>
					<td><s:date name="CCreateDate" format="dd-MM-yyyy"/></td>
					<td>
						<s:if test="#session.fun713 == 1">
