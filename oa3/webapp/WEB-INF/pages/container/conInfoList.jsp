...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript">
		...
		//添加
		function add(){
			document.getElementById("queryForm").action = "${ctx}/container/AddConInfo.action";
			document.getElementById("queryForm").submit();
		}
		//修改
		function edit(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/container/EditConInfo.action";
			document.getElementById("queryForm").submit();
		}
		//查看
		function view(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/container/ViewConInfo.action";
			document.getElementById("queryForm").submit();
		}
		//删除确认
		function confirmDel(cid){
			if (confirm("删除的数据将无法恢复，是否确认删除？")){
				document.getElementById("cid").value = cid;
				document.getElementById("queryForm").action = "${ctx}/container/DeleteConInfo.action";
				document.getElementById("queryForm").submit();
			}
			return false;
		}
  ...
  <body>
  	<div class="right">
  		<div class="right_1">
			<form id="queryForm" action="${ctx}/container/ConInfoList.action" method="post">
        ...
        <div class="right_3">
	        <table id="table1" class="table" style="width:680px;">
	            ...
	            <s:iterator value="#request.conInfoList" var="conInfoList" status="num">
	            <tr>
					<td><s:property value="(#num.index+1)+(pageNow-1)*pageSize" /></td>
					<td><s:property value="CCode"/></td>
					<td><s:property value="TContainerModel.CCode"/></td>
