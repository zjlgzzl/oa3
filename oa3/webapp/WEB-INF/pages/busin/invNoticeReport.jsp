...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			var cusid = "${requestScope.invUserQuery.cusid}";
		...
		function forward(flag){
			document.getElementById("queryForm").action = "${ctx}/busin/InvNoticeReport.action";
			//首页
			if (flag == 1){
		...
		function updateReply(businId, rowIndex){
			...
			var url="${ctx}/busin/UpdateReply.action?archiveRemarks="+encodeURI(reply)+"&businId="+businId;
			$.ajax({
				type:"Post",
				url:url,
  ...
  <body>
  <form id="queryForm" action="${ctx}/busin/StopNotice.action" method="post">
	  	<div class="right">
	  		<div class="right_1">
				<table style="width:950px; margin-left:0;">
					<tr>
						...
						<td style="text-align:left; width:120px;">
							<input name="invUserQuery.ccode" value="${requestScope.invUserQuery.ccode}"
									class="field border" style="width:120px;"/>
	  					</td>
	  					<td style="width:70px;">客户名称</td>
						<td style="width:340px;text-align:left; ">
							<select id="cusid" name="invUserQuery.cusid" class="border" style="width:340px;border:1px solid lightblue;">
	        ...
	        <div class="right_3">
		        <table id="table1" class="table" style="width:1100px;">
		           	<tr>
		               	...
		               	<th style="width:200px;">开票提醒</th>
		               	<th style="width:120px;">提醒人</th>
		            ...
		            <s:iterator value="#request.businList" var="businList" status="num">
			            <tr>
							...
							<td><s:property value="noticeRemarks"/></td>
							<td><s:property value="lastMotifyName"/></td>
							<td><s:property value="lastNoticeName"/></td>
							<td>
								<input id="reply<s:property value="#num.index" />"
											type="text" value="${noticeReply}" class="border field" style="width:186px;"
											onchange="updateReply(<s:property value="businId" />,<s:property value="#num.index" />)"/>
							</td>
							<td>
								<s:if test="#session.fun315==1">
									<a href="javascript:void(0);" 
											onclick="stopNotice(<s:property value="businId"/>);">取消提醒</a>
								</s:if>
							</td>
						</tr>
		            </s:iterator>
