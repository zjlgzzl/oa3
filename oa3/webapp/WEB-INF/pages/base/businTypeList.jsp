...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript">
		...
		//添加
		function add(){
			document.getElementById("queryForm").action = "${ctx}/btype/AddBusinType.action";
			document.getElementById("queryForm").submit();
		}
		//修改
		function edit(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/btype/EditBusinType.action";
			document.getElementById("queryForm").submit();
		}
		//查看
		function view(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/btype/ViewBusinType.action";
			document.getElementById("queryForm").submit();
		}
		//删除确认
		function confirmDel(cid){
			if (confirm("删除的数据将无法恢复，是否确认删除？")){
				document.getElementById("cid").value = cid;
				document.getElementById("queryForm").action = "${ctx}/btype/DeleteBusinType.action";
				document.getElementById("queryForm").submit();
			}
			return false;
		}
  ...
  <body>
  	<div class="right">
  		<div class="right_1">
			<form id="queryForm" action="${ctx}/btype/BusinTypeList.action" method="post">
					<table style="width:430px; margin-left:0;">
						<tr>
		  					<td style="width:100px;">服务单类型名称</td>
							<td style="width:120px;text-align:left; ">
		  					...
		  					<td>
								...
								<input type="button" value="添加" class="btn_img" style="margin-left:10px;"
										onclick="add();"/>
								<input type="hidden" id="cid" name="typeId" />
        ...
        <div class="right_3">
	        <table id="table1" class="table">
	           	<tr>
	               	...
	               	<th style="width:200px;">服务单类型名称</th>
	               	<th style="width:100px;">状态</th>
	               	<th style="width:250px;">备注 </th>
	                <th style="width:210px;">操作</th>
	            </tr>
	            <s:iterator value="#request.businTypeList" var="businTypeList" status="num">
	            <tr>
					...
					<td>
						<s:if test="CCreateUserid==#session.userID">
