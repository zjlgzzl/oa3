  ...
  <head>
	...
	<script type="text/javascript">
		...
		function returnList(){
			...
			if (ctype == 1){
				document.getElementById("editForm").action = "${ctx}/cost/CostItemList.action";
			}else{
				document.getElementById("editForm").action = "${ctx}/cost/CashItemList.action";
			}
		...
		function check(){
			...
			var ctype = document.getElementById("ctype").value;
			if (ctype == 1){	
				document.getElementById("editForm").action = "${ctx}/cost/UpdateCostItem.action";
			}else{
				document.getElementById("editForm").action = "${ctx}/cost/UpdateCashItem.action";
			}
  ...
  <body>
  	<div>
  		<div style="background:#FFF;margin-top:50px;margin-left:20px;width:1000px;">
			<form id="editForm" action="${ctx}/cost/SaveCostItem.action" method="post">
		        <table style="width:720px;">
		        	<tr style="height:50px;">
		        		<td style="width:120px;text-align:center;">
  							<span style="color:red;">*</span>
			           		<s:if test="#request.cost.CType == 1">
			           			成本项目名称
			           		</s:if>
			           		<s:else>
			           			回款项目名称
  						...
  						<td style="width:600px;text-align:left;">
  							<input id="CName" name="cost.CName" value="${requestScope.cost.CName}"
		           					type="text" class="field border" />
		           			<input id="CId" name="cost.CId" value="${requestScope.cost.CId}" type="hidden" />
							<input id="CCreateUserid" name="cost.CCreateUserid" value="${requestScope.cost.CCreateUserid}" type="hidden" />
				           	<input id="CCreateDate" name="cost.CCreateDate" value="${requestScope.cost.CCreateDate}" type="hidden" />
				           	<input id="CState" name="cost.CState" value="${requestScope.cost.CState}" type="hidden" />
				           	<input type="hidden" id="ctype" name="cost.CType" value="${requestScope.cost.CType}" />
		        	...
		        	<tr style="height:50px;">
  						...
  						<td style="width:600px;text-align:left;">
  							<textarea id="remarks" name="cost.CRemarks" rows="5" cols="10" style="width:182px;">${requestScope.cost.CRemarks}</textarea>
  						</td>
  					</tr>
		        </table>
		        <s:if test="#request.cost.CType == 1">
