  ...
  <head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			var completeFlag = "${requestScope.reQuery.payComplete}";
			select = document.getElementById("completeFlag");
			for(var i=0;i<select.options.length;i++){
				if(select.options[i].value == completeFlag){
					select.options[i].selected = true;
					break;
				}
			}
		...
		function forward(flag){
			document.getElementById("queryForm").action = "${ctx}/report/BusinReceiveDReport2.action";
			//首页
			if (flag == 1){
		...
		function confirmArchive(businId){
			if (confirm("确认解除归档？")){
				...
				document.getElementById("queryForm").action = "${ctx}/report/CancelScheduleArchive.action";
				document.getElementById("queryForm").submit();
			}
		}
		...
		function expFile(){
			document.getElementById("queryForm").action = "${ctx}/report/ExpRecieveReport2.action";
			document.getElementById("queryForm").submit();
		}
		function complete(flag, businid){
			var msg = "确认完结？";
			if (flag == 0){
				msg = "确认解除完结？";
			}
			if (confirm(msg)){
				document.getElementById("businId").value = businid;
				document.getElementById("payCompleteFlag").value = flag;
				document.getElementById("queryForm").action = "${ctx}/report/PayComplete.action";
				document.getElementById("queryForm").submit();
			}
		}
		function selectItem(){
		...
		function complete2(flag){
			if (confirm("确认完结？")){
				document.getElementById("payCompleteFlag").value = flag;
				document.getElementById("queryForm").action = "${ctx}/report/PayComplete2.action";
				document.getElementById("queryForm").submit();
			}
		}
		function cancelNotice(businId){
			document.getElementById("businId").value = businId;
			document.getElementById("queryForm").action = "${ctx}/report/CancelNotice.action";
			document.getElementById("queryForm").submit();
		}
	</script>
  </head>
  <body>
  	<form id="queryForm" action="${ctx}/report/BusinReceiveDReport2.action" method="post">
	  	<div class="right">
	  		<div class="right_1">
				<table style="width:2180px; margin-left:0;">
					<tr>
						...
						<td style="width:100px;text-align:left; ">
	  					...
	  					<td style="width:90px;">D.N OR INV</td>
						<td style="width:100px;text-align:left; ">
							<input name="reQuery.nodate5" value="${requestScope.reQuery.nodate5}" 
									type="text" class="field border" style="width:100px;" />
	  					</td>
	  					<td style="width:70px;">是否完结</td>
	  					<td style="width:100px;text-align:left; ">
							<select id="completeFlag" name="reQuery.payComplete" class="border" style="width:100px;">
				           		<option value="-1">全部</option>
						...
						<td style="width:80px;text-align:left; ">
	  					...
	  					<td style="text-align:left;">
	  						<label style="margin-left:10px;">排序规则：按差额</label>
	  						<select id="orderRule" name="orderRule" class="border" style="width:80px;">
							...
							<input type="hidden" id="payCompleteFlag" name="payCompleteFlag" />
						</td>
						<td style="text-align:right;padding-right:170px;">
							<s:if test="#session.fun624 == 1">
								<input type="submit" value="批量完结" class="btn_img" style="margin-left:80px;"
										onclick="complete2(1);"/>
							</s:if>
						</td>
					</tr>
	        ...
	        <div class="right_3">
		        <table id="table1" class="table" style="width:2220px;">
		           	<tr>
		               	<!--  
		               	...
		               	<th style="width:130px;">发票1</th>
		               	<th style="width:130px;">Re-Im</th>
		               	<th style="width:130px;">发票2</th>
		               	<th style="width:90px;">Tax INV</th>
		               	<th style="width:130px;">含税发票</th>
		            ...
		            <s:iterator value="#request.receiveList" var="receiveList" status="num">
			           <s:if test="acNotice==1">
			            ...
			            <s:else>
							...
							<td>
								<s:if test="#session.fun225 == 1 && acNotice == 1">	
									<a href="javascript:void(0);"
													onclick="cancelNotice(<s:property value="businId"/>);">取消提醒</a>
								</s:if>
								<s:if test="#session.fun624 == 1">
									<s:if test="payComplete==0">
										<a href="javascript:void(0);"
													onclick="complete(1,<s:property value="businId"/>);">完结</a>
									</s:if>
									<s:if test="payComplete==1">
										<a href="javascript:void(0);"
													onclick="complete(0,<s:property value="businId"/>);">解除完结</a>
									</s:if>
								</s:if>
								<s:if test="#session.fun618==1 && payComplete==0">
									<s:if test="schedule_archive==1">
