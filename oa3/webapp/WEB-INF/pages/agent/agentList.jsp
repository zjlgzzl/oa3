...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript">
		...
		//添加
		function add(){
			document.getElementById("queryForm").action = "${ctx}/agent/AddAgent.action";
			document.getElementById("queryForm").submit();
		}
		//修改
		function edit(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/agent/EditAgent.action";
			document.getElementById("queryForm").submit();
		}
		//删除确认
		function confirmDel(cid){
			if (confirm("删除的数据将无法恢复，是否确认删除？")){
				document.getElementById("cid").value = cid;
				document.getElementById("queryForm").action = "${ctx}/agent/DeleteAgent.action";
				document.getElementById("queryForm").submit();
			}
			return false;
		}
		...
		function view(cid){
			document.getElementById("cid").value = cid;
			document.getElementById("queryForm").action = "${ctx}/agent/ViewAgent.action";
			document.getElementById("queryForm").submit();
		}
	</script>
  </head>
  <body>
  	<div class="right">
  		<div class="right_1">
			<form id="queryForm" action="${ctx}/agent/AgentList.action" method="post">
				<table style="width:800px; margin-left:0;">
					<tr>
	  					<td style="width:70px;">代理</td>
