...
function addContainer(count){
	...
	td4.innerHTML = '<select id="conType'+conCount+'" name="cons['+conCount+'].CContainerType" style="width:76px;" class="field border">' +
...
function addCost(count){
	...
	//应付对象
	var td666 = document.createElement("td");
	td666.innerHTML = '';
	tr.appendChild(td666);
	var td66 = document.createElement("td");
	
	td66.innerHTML = '<select id="agent'+costCount+'" name="costs['+costCount+'].agent" style="width:113px;" class="field border _select">' +
								'<option value="">PLEASE CHOOSE</option></select>'
	tr.appendChild(td66);
	
	//应付时间
	var td777 = document.createElement("td");
	td777.innerHTML = '';
	tr.appendChild(td777);
	var td7777 = document.createElement("td");
	
	td7777.innerHTML = '<input id="payDate'+costCount+'" name="costs['+costCount+'].payDateStr"  pattern="dd-MM-yyyy"' + 
									'type="text" class="field border" style="width:80px" onclick="WdatePicker()" readonly="readonly"/>'
	tr.appendChild(td7777);
	//备注
	var td7 = document.createElement("td");
	td7.innerHTML = '';
	tr.appendChild(td7);
